...
```{r}
object <- RunAzimuth(object, reference = "pancreasref")

azimuth_prediction <- object@meta.data$predicted.annotation.l1

write.csv(azimuth_prediction, file = "./azimuth_human_pancreas.csv", row.names = FALSE)
```

# Preprocessing
```{r}
# Add the new labels to the Seurat object's metadata
object@meta.data$predict_labels <- azimuth_prediction
...
object_integrated_ss <- obj.list %>%
 Run.STACAS(dims = 1:ndim, cell.labels = "predict_labels", label.confidence = 1)
...
write.csv(pca_embeddings_ordered, file = "./human_pancreas_stacas_embeddings_azimuth.csv", row.names = TRUE)
```
