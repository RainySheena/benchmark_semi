...
```{R}
object <- readRDS("../macaque_raw.rds")
```
...
hard_label <- read.csv('./labels/hard_label_macaque_30.csv', row.names = 1)
```
...
write.csv(pca_embeddings_ordered, file = "./embeddings/macaque_stacas_embeddings_randomly_missing_labels_30.csv", row.names = TRUE)
```
