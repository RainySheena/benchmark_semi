...
batch_labels <- read.csv('./batch_labels_lung_atlas_70.csv', row.names = 1)
```
...
write.csv(pca_embeddings_ordered, file = "./lung_atlas_stacas_embeddings_partially_annotated_batches_70.csv", row.names = TRUE)
```
...
p2_ss <- DimPlot(object_integrated_ss, group.by = "cell_type") + 
 theme(aspect.ratio = 1) + ggtitle("Cell labels after semi-supervised integration")
