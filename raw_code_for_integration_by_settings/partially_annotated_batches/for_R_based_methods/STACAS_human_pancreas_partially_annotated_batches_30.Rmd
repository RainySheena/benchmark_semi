...
```{R}
Convert("./human_pancreas.h5ad", dest = "h5seurat", overwrite = TRUE)
...
batch_labels <- read.csv('./batch_labels_human_pancreas_30.csv', row.names = 1)
```
...
write.csv(pca_embeddings_ordered, file = "./human_pancreas_stacas_embeddings_partially_annotated_batches_30.csv", row.names = TRUE)
```
...
```{r}
# Save to CSV file
write.csv(final_annotations_ordered, file = "./human_pancreas_stacas_embeddings_final_annotations_ordered.csv", row.names = FALSE)
```
